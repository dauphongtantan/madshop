	...
	@endif	
		...
		<tbody>
	 	...
	 	@foreach($orders as $key =>$dh)	
				...
				<td>
					<a href="{{ route('viewOrders',['id'=>$dh->id])}}" class="label label-md label-info">Xem</a>
					<a href="{{ route('deleteOrders',['id'=>$dh->id])}}" class="label label-md label-danger" onclick="return confirm('Bạn chắc chắn chứ?')">Hủy</a>
				</td>
			</tr>	
